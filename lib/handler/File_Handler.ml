...
let get_directory req =
  ...
  let* result = File_Service.get_directory_files ~path:(Config.get_data_directory ()) ~mime_filter () in
  match result with
...
let delete_directory _req =
  let* result = File_Service.delete_directory (Config.get_data_directory ()) in
  match result with
...
let scan_directory _req =
  let* files = File_Service.scan_directory (Config.get_data_directory ()) in
  match files with
  | Error _ -> Dream.respond ~status:`Internal_Server_Error "internal server error"
  | Ok files -> 
    let json = `List (List.map File.file_to_json files) in
