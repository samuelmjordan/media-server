...
let insert ?(db_uri = None) file =
  let conn_fn = match db_uri with 
    | Some uri -> Db.with_connection_uri uri
    | None -> Db.with_connection in
  let* result = conn_fn (fun (module Db : Caqti_lwt.CONNECTION) ->
