module Q = struct
  ...
  let get_all_media_metadata =
    Caqti_type.unit ->* Caqti_type.(t12 File.File_Uuid.caqti_type bool string int64 string string string float string string string bool)
    @@
    "SELECT file_id, adult, backdrop_path, tmdb_id, original_language, original_title, overview, popularity, poster_path, release_date, title, video FROM media_metadata_"
