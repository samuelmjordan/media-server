module Q = struct
  ...
  let insert_media_metadata =
    ...
    @@
    "INSERT INTO media_metadata (file_id, adult, backdrop_path, tmdb_id, original_language, original_title, overview, popularity, poster_path, release_date, title, video) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ...
  let get_media_metadata =
    ...
    @@
    "SELECT file_id, adult, backdrop_path, tmdb_id, original_language, original_title, overview, popularity, poster_path, release_date, title, video FROM media_metadata WHERE file_id = ?"

  let delete_media_metadata =
    File.File_Uuid.caqti_type ->. Caqti_type.unit
    @@
    "DELETE FROM media_metadata WHERE file_id = ?"
  ...
  let update_media_metadata =
    ...
    @@
    "UPDATE media_metadata SET adult = ?, backdrop_path = ?, tmdb_id = ?, original_language = ?, original_title = ?, overview = ?, popularity = ?, poster_path = ?, release_date = ?, title = ?, video = ? WHERE file_id = ?"
end