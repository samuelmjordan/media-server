...
let test_create_and_find _switch () =
  ...
  let* create_result = User_Repository.create ~user_id ~name:"alice" ~email:"alice@example.com" () in
  (match create_result with
   | Ok () -> ()
   | Error e -> Alcotest.fail (Caqti_error.show e));
  let* find_result = User_Repository.find_by_id ~user_id () in
...
let test_find_nonexistent _switch () =
  ...
  let* result = User_Repository.find_by_id ~user_id:fake_id () in
  match result with
  | Ok None -> Lwt.return ()
...
let test_find_all _switch () =
  ...
  let* create1_result = User_Repository.create ~user_id:user1_id ~name:"alice" ~email:"alice@example.com" () in
  (match create1_result with Ok () -> () | Error e -> Alcotest.fail (Caqti_error.show e));
  let* create2_result = User_Repository.create ~user_id:user2_id ~name:"bob" ~email:"bob@example.com" () in
